...
public class ScreenRecorder extends Thread {
    ...
    private RequestCameraRender mCameraListener;
    ...
    public ScreenRecorder(int width, int height, int bitrate, int dpi, int audioSampleRate, int audioBitrate, String dstPath, ScreenRecordListener listener, RequestCameraRender cameraListener) {
        super(TAG);
        mWidth = width;
        mHeight = height;
        mBitRate = bitrate;
    ...
    public void run() {
        try {
            ...
            mCameraListener.setCameraRender(mSurface);
    ...
    private void recordVirtualDisplay() {
        while (!mQuit.get()) {
            ...
            if (mEncoder != null) {
                ...
                boolean timeStampIssue = (mBufferInfo.presentationTimeUs < mLastTimestamp);
                long tmp = mLastTimestamp;
                mLastTimestamp = mBufferInfo.presentationTimeUs;
                if (timeStampIssue) {
                    Log.w(TAG, String.format("skipping buffer because timestamp suspicious : %d vs %d",
                            mBufferInfo.presentationTimeUs, tmp));
                    continue;
                }
            ...
            if (mAudioRecord != null && mAudioRecord.getState() == AudioRecord.STATE_INITIALIZED && mAudioEncoder != null) {
                ...
                if (size > 0) {
                    ...
                    if (audioInputBufferIndex >= 0) {
                        ...
                        inputBuffer.put(mAudioBuffer, 0, Math.min(mAudioBuffer.length, inputBuffer.limit()));
                        mAudioEncoder.queueInputBuffer(audioInputBufferIndex, 0, Math.min(mAudioBuffer.length, inputBuffer.limit()), timestamp, 0);
                    }
                }
    ...
    public interface RequestCameraRender{
        void setCameraRender(Surface surface);
    }
}
